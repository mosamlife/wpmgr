...
- **Three backup destinations**: Control-plane-managed bucket, customer-owned S3-compatible bucket (agent never holds the credentials), or a local folder on the WordPress host. Chunks travel over TLS and are stored as uploaded, so the security of a backup is the security of the destination you pick; a bucket you own or a local folder keeps the bytes on storage you control, which is a different property from encryption at rest and does not stand in for it. See [docs/features/backups.md](./docs/features/backups.md#backup-storage-and-encryption).
