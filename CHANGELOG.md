...
- There was nothing here for an operator to have noticed. No code changed and no behaviour changed on either side of it: a live advisory database was updated overnight, and the same commit that passed CI in the evening failed the next morning. So a build of the previous release carries these whenever and however you built it, and a running install shows no symptom that would tell you. The toolchain a binary was built with is readable out of the binary itself (`go version -m` against the `wpmgr` binary inside the image), which is the only reliable way to tell what you are running.
