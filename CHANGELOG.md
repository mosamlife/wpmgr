...
- Objects already orphaned before this release are NOT cleaned up by it, and nothing in this release finds them. The reclamation record is written by the delete, so it only exists for sites deleted from this version onwards. Sites you deleted on an earlier version left no record anywhere, which is the whole defect, so those manifests are still in your bucket and still on your storage bill: the account that reported this has 90 of them right now. Clearing them is a deliberate step you take yourself, against `tenant/<tenant id>/site/<site id>/`, one folder per deleted site, with the literal `site/` segment in the middle. Rather than deleting by hand you can hand a known-deleted site to the sweep with a single insert, which keeps every safety check in play, including its refusal to touch a site whose record still exists; the exact statement is in the `m113_site_object_reclaim` migration. Get the kind field in that statement wrong and the database now refuses the whole insert, on the spot, naming the constraint. It used to accept it, and the sweep then closed the record as one it could not act on, so a single typo in the statement meant to recover those files stranded them for good.
- A site whose backups go to your own storage bucket has its manifests swept like any other. That case was previously described as left alone, which was wrong: manifests are always written to the bucket this instance itself is configured with, whatever destination the backup payload was sent to, so they are all in scope and none of them are stranded. What this instance does not touch is the backup payload sitting in your own bucket, which it holds no credentials for; the reclamation record carries which destination the site used purely so the log can say which of the two it was.
- Deleting an empty organisation no longer destroys the reclamation records of the sites it used to hold. Deleting an organisation with no sites and no other members removes it immediately and frees no object storage, on the reasoning that an organisation with no sites owns nothing; an organisation whose sites were all deleted first fits that description exactly and does own objects. The records are now kept independently of the organisation row, so the sweep still finds and clears those folders afterwards. The same applies when an organisation is removed by the orphan cleanup that follows deleting the last account in it.
- Somebody invited to a single site can no longer read or alter the reclamation records naming every other site in the organisation. Marking another site's record complete would have left that site's manifests in storage with nothing left to name them, which is the original defect reached through a different door. The database now enforces this itself, the same way it does for site email settings, rather than leaving it to whichever page happens to ask.
- An account that lost its last backup-carrying site stopped being garbage collected at all, so its stored chunks leaked as well as its manifests. The collector's account list came only from completed backups, and deleting the site holding the final one removed the account from that list permanently. It now also considers accounts that still have stored chunks, which is enumeration only: every existing safety check on what may actually be deleted is unchanged, and a backup running at the time still protects everything it touches.
