...
|---|---|
| 062–063 | Claimed by an open, unmerged PR (#519, `docs/adr-061-amendments-and-adr-062`) at the time ADR-064 was filed: ADR-062 is the assistant surface's Phase 2 (content operations), ADR-063 is licensing and third-party reuse. Neither has a file on `main` yet, so ADR-064 was filed ahead of them rather than reused once they land — run `gh pr view 519 --json files -q '.files[].path'` to confirm what that PR currently adds. |
