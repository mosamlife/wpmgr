# ADR-037 — WPMgr feature-parity roadmap (WPvivid + WPRemote)

**Status:** proposed
**Date:** 2026-05-29
**Sources:** ADR-036 (WPvivid-parity, separate doc) + 8 parallel research reports
- D1 storage adapters · D2 backup engine · D3 restore engine · D4 UI/UX (WPvivid)
- E1 wire protocol · E2 site health · E3 security suite · E4 ops + extensibility (WPRemote)
...
Two competitors, two roadmaps, one merged plan.

- **WPvivid** is backup-focused. ADR-036 covers the backup-restore parity gap (URL rewriter, storage destinations, mu-plugin trap, audit receipt, etc.).
- **WPRemote** (BlogVault) is a full ManageWP-class agent. ADR-037 (this doc) covers the broader site-management gap (diagnostics, error monitoring, activity log, security suite, multi-account, dynamic sync, form testing, whitelabel).
- Where WPMgr already leads BOTH (Ed25519 + canonical message + JTI replay + age E2E encryption + 60s autologin + hash-chained audit log + Schema::ensureCurrent + chunk dedup) — no work needed.
- The 10 sprints below are prioritized so the most operator-visible value lands first.

---

## Position vs WPRemote — where WPMgr already leads (no work needed)

| Dimension | WPMgr | WPRemote |
|---|---|---|
| Inbound auth | Ed25519 JWT + `aud`/`cmd` claims + jti replay table | RSA/Ed25519 + bundled `m_public.pub` + 5-min replay floor |
| Outbound auth | Ed25519 detached signature over canonical message | `sha1($pub.$secret.$time.$version)` querystring |
...
| Autologin recovery | Ed25519 JWT, ≤60s, single-use, role-bounded | 24h `mt_rand` weak secret in HTML form |
...
## Position vs WPvivid — where WPMgr already leads (no work needed)

| Capability | WPMgr | WPvivid free |
|---|---|---|
...
| Restore confirm | typed-host destructive modal | `confirm("Are you sure?")` |
...
| Restore safety (self-DoS, disk bomb) | Shipped (Track 1, v0.9.5) | WPvivid in-place delete-extract |
...
1. **130+ inbound actions vs WPMgr's 8.** WPRemote has reusable primitives (fs/db/manage/info wings) so CP can do almost anything; we have purpose-built commands.
2. **No streaming chunked protocol.** Their `WPRStream` (CRC32/MD5 chunk prefixes + multipart push directly into open CP sockets). We `wp_remote_request` PUT only.
3. **No multi-account / multi-tenant agent.** `bvAccountsList` allows N CPs per site. Strict 1:1 today.
4. **Sparse site fingerprint.** Our metadata = 8 fields. WPRemote = ~50.
...
- **No error monitoring.** WPRemote has `bv_php_error_store` (md5 dedup, 10k cap, CP pull-and-purge). We have nothing.
- **15 leapfrog opportunities** (NEITHER WPRemote nor WPvivid ships):
  1. PHP-EOL countdown
...
- **Multisite likely-broken bug** — our `Settings` uses plain `get_option`/`update_option` with no `get_site_option` fallback. WPRemote does the two-tier resolution.
...
- `wpmgr_activity_log` table (port WPRemote's 30 hook handlers — MIT-rewrite, not GPL'd copy)
...
- `class-2fa.php` (RFC-6238 TOTP, AES-256-GCM secrets keyed off keystore, **WITH backup codes** — WPRemote omits these)
...
- Port `WPRStream` design with our JWT signing
...
- `BVFormTesting`-equivalent for 6 form plugins — bypass CAPTCHA + abort email
- Login screen whitelabel (data-URI-only logo + banner via `login_message` filter — no remote URLs)
- Tightened recovery channel (15-min QR vs 24h `mt_rand` weak random in HTML)

### Sprint 8 — Dynamic sync + incremental backup (4-6 weeks, the BIG bet)
- Port ~150 WP+WC hook listeners → `wpmgr_dynamic_sync` table
...
- Unlocks the WooCommerce real-time story which is WPRemote's strongest non-Sucuri differentiator
...
- Strategy 2 (data-driven JSON-AST engine matching WPRemote) — 6-9 weeks
...
| Pattern | Why skip |
|---|---|
| `gtsvr` raw `$_SERVER` dump | Info leak; ship curated allowlist instead |
| `misc.phpinfo` direct dump wire | Security smell |
| `scrty` 2FA secret writing wire | Secrets never transmitted; locally generated only |
| `brand` plugin hide-from-menu | Antagonistic to operator transparency |
...
| 24h `mt_rand` recovery secret in HTML | Tighten to 15-min QR mintable on demand |
| WPRemote's per-user opt-in 2FA without backup codes | Ship WITH backup codes from day 1 |
| Recursive `_upload()` retry stack-grows | Anti-pattern; bounded loop |
| Base64 credential "encryption" | Real age encryption (already in keystore) |
| Colocated WP-admin HTML per adapter | We have React |
| `send_to_site` cross-site migration | Out of scope for backup destinations |

### From WPvivid
| Pattern | Why skip |
|---|---|
| PclZip fallback for hosts without ext-zip | Modern managed hosts ship ext-zip; maintenance burden not worth it |
| `WPvivid_Crypt` (RSA + Rijndael custom format) | Superseded by age + ChaCha20-Poly1305 |
| Snapshot module (in-DB shadow tables as "git tags") | We have chunk-level dedup; in-DB shadow doubles MySQL storage |
| `is_merge` / `backup_all.zip` mega-merge step | Re-pack with no compression upside, doubles transient disk |
| Per-zip `wpvivid_package_info.json` (env info INSIDE every zip) | We have a CP-side manifest; duplicating splits source-of-truth |
| `compress_file_use_cache` toggle | Footgun; always stream |
