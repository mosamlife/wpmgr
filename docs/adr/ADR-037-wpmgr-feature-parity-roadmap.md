...
- Change tracking is scoped to content/config tables only — posts, postmeta, options, terms, theme mods. `users` is excluded by default, for a stronger version of the same reason transactional tables are excluded below: a stale staging row can carry a password hash production has since rotated past, a role change production reverted or never had, or an account that only ever existed in staging, and a push-back can silently clobber a production account created during the staging window. Staging is also typically held to a lower security bar than production, so round-tripping `users` turns a staging compromise into a path to production identity. A narrow, explicitly-scoped user push-back could be added later with its own field allowlist, a scrubbing rule, and an explicit statement about credentials and roles, but the default here is exclusion. Transactional tables (orders, comments, form submissions, new signups) are never tracked for push-back and are never written to by one either; this is what keeps live production data safe from a stale staging clone
...
- Push-back writes an audit receipt, reusing the hash-chained audit log (shipped, Sprint 3). The restore audit receipt itself is still pending per the D1-D4 gaps above (ADR-036) — this sprint depends on it landing first
...
- **Sizing.** No single existing sprint is a clean comparable, so this is a range, not a point estimate. The clone-only base is shaped like Sprint 2/3 (new table + CP endpoints + dashboard route on top of already-shipped machinery, 1.5-2 weeks). Round-trip adds a change-tracking and conflict-detection layer that is the same *kind* of problem Sprint 10 solves at full scale (hook listeners + delta semantics, 4-6 weeks) but deliberately narrower — a fixed set of content/config tables, not ~150 hooks and not WooCommerce CDC — plus a diff/merge review UI and destructive-action safety that reuse the already-shipped typed-host confirm pattern instead of inventing a new one. That puts the added scope well under Sprint 10's full bet but clearly above the original clone-only estimate: **3-5 weeks** total. That estimate assumes the restore audit receipt (D1-D4 above, still pending) has landed by the time this sprint starts; it is a dependency of this sprint, not part of its own 3-5 weeks.
