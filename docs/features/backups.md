...
Chunks are not encrypted on the agent in shipped builds. They are stored as uploaded at the destination configured for the site, so a backup is protected by whatever protects that destination.

What that means differs by destination, and by whether the control plane is one you self-host or the hosted service:

| Destination | Who holds the chunks | Transport | Encryption at rest |
|---|---|---|---|
| Control-plane-managed bucket (**the default**) | The control plane's object storage, and anyone with access to it. On a self-hosted control plane that storage is yours; on the hosted service it is operated by WPMgr. | Uploaded over HTTPS to a presigned URL | Whatever that object storage is configured to provide |
| Customer-owned S3-compatible bucket | Your bucket, on storage you control | Uploaded over HTTPS to a presigned URL | Whatever you have enabled on the bucket |
| Local folder on the WordPress host | The site's own server, on storage you control | None; the agent writes to disk | None from WPMgr. Whatever the host's own disk or filesystem provides, which WPMgr does not configure and cannot guarantee |

Keeping the bytes on operator-controlled storage is a different property from encryption at rest and does not stand in for it. Treat backup storage the way you would treat any store holding a full copy of the site's files and database: restrict access to the bucket or directory, and turn on the destination's own server-side encryption where it offers one.
...
The site's age keypair is generated by the agent itself and the public recipient is pushed to the control plane automatically as part of the agent's normal metadata sync, so in practice no operator setup is required. Starting a backup on a site that has not yet synced a recipient (for example, in the first moments after enrollment, or on a very old agent build) returns a 422 with code `age_recipient_missing`. Retrying once the site's next metadata push completes resolves it.

The `age_recipient` field on a snapshot records the recipient the site had published at backup time. It is stored for provenance.
...
independent of the schedule. Because chunks are stored as uploaded, this
setting is what decides who can read a backup.

### The three destination kinds

| Kind | Where bytes go | Who sees the bytes |
|---|---|---|
| **Managed (control plane)** | The control plane's own object storage. Default; unchanged from earlier versions. | The control plane's storage backend, and anyone with access to it. |
...
writes chunks straight to disk on the site's own server, and the control
...
difference is where the agent fetches chunks from: a local path on its own
filesystem, or a presigned GET URL for control-plane or S3-compatible
storage.
...
```jsonc
{
  ...
  "age_recipient": "age1...",      // recipient the site had published, for provenance
...
| `encrypting_uploading` | Chunk upload in progress (the phase name is the wire value) |
...
Enqueue a restore. The control plane resolves the selection, issues presigned GET URLs, and dispatches a signed restore command to the agent. The agent downloads each chunk, verifies it against its content hash, and reassembles.
