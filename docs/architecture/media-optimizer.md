...
> **Credit.** Orchestration patterns (the postmeta blob shape, the
> original-rename trick, the Accept-header `.htaccess` fallback, the
> serialized-safe URL rewriter) are inspired by FlyingPress 5.4.5 (FlyingWeb);
> **no FlyingPress code is included or copied**. Encoding runs on WPMgr's own Go
> control plane using Discord's `lilliput` (MIT). See
> [apps/agent/NOTICE.md](../../apps/agent/NOTICE.md).
