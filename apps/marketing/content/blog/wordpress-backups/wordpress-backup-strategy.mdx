...
WPMgr keeps destination credentials in the control plane rather than on the sites, and supports S3-compatible and local destinations configured once at the control plane and reused across the fleet. Client-side encryption before upload is planned; until it ships, choose a destination that encrypts at rest so the first property above still holds.
