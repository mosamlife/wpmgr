...
export const HOME_FAQ: FaqItem[] = [
  ...
  },
  {
    ...
    a: "If you self-host the control plane, your data stays on your own infrastructure. On hosted plans with managed backup storage, backups default to a control-plane-managed bucket that WPMgr operates; on the Free plan, which has no managed bucket, a site needs its own S3-compatible bucket or a local folder configured before backups will run. Either way, pointing a site at your own S3-compatible bucket or a local folder on its own server keeps the chunks off the control plane instead. Nothing is encrypted client-side today: a control-plane-managed bucket protects data however that storage is configured, your own S3-compatible bucket protects it however you have configured that, and a local folder gets no encryption from WPMgr at all, only whatever the host's own disk provides. Image bytes move directly between your site and your storage, and the agent redacts emails, passwords, secrets, tokens, and salts before any diagnostics ever leave a site.",
  },
  {
